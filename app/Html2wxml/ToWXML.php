...
class ToWXML
{
    ...
    public function towxml($text, $args = [])
    {

        $this->init();
        $this->args = $this->param_atts([
            'type' => 'html',
            'highlight' => true,
            'linenums' => true,
            'imghost' => null,
            'encode' => true,
            'highlight_languages' => ['html', 'js', 'php', 'css']
        ], $args);
        ...
        } else if (in_array($this->args['type'], ['markdown', 'md'])) {
    ...
    public function html2array($html, $remove_line_break = true)
    {
        ...
        return isset($json['nodes'][0]['nodes']) ? $json['nodes'][0]['nodes'] : [];
    }
    ...
    public function element2array($element, $remove_line_break = true)
    {
        ...
        $obj = compact('tag');
        if ($type != null) {
            $obj = array_merge($obj, compact('type'));
        }
